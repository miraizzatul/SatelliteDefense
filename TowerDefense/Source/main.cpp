...
int main(int argc, char* argv[])
{
	...
	//enemies.emplace_back(path); // add one enemy for now
	//
	////------------end enemies--------------//
	...
	//towers.emplace_back(100.f, 250.f, 50.f, 100.f); // x, y, size, range

	//// For movement testing: make one tower movable
	//SDL_FRect towerRect = { 400.f, 200.f, 40.f, 40.f };
	...
	//	}
	//	//movement logic
	//	if (keys[SDL_SCANCODE_W] || keys[SDL_SCANCODE_UP]) towerRect.y -= speed * deltaTime;
	//	if (keys[SDL_SCANCODE_S] || keys[SDL_SCANCODE_DOWN]) towerRect.y += speed * deltaTime;
	//	if (keys[SDL_SCANCODE_A] || keys[SDL_SCANCODE_LEFT]) towerRect.x -= speed * deltaTime;
	//	if (keys[SDL_SCANCODE_D] || keys[SDL_SCANCODE_RIGHT]) towerRect.x += speed * deltaTime;

	//	//Clamp the tower from going offscreen
	//	if (towerRect.x < 0) towerRect.x = 0;//left edge
	//	if (towerRect.y < 0) towerRect.y = 0;//top edge
	//	if (towerRect.x + towerRect.w > 800)towerRect.x = 800 - towerRect.w;//(Tower.x+Tower.w) right edge of rectangle
	//	if (towerRect.y + towerRect.h > 600)towerRect.y = 600 - towerRect.h;//(Tower.y+Tower.h) bottom edge of rectangle
